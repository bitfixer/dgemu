...
{
	valFlag(ctx, F_C, (1 - (byte)getFlag(ctx, F_C) != 0));
	resFlag(ctx, F_N);
...
{
	ctx->tstates += 5;
	int carry = getFlag(ctx, F_C);
	byte value = doCP_HL(ctx);
	if(getFlag(ctx, F_H))
		value--;
	WR.HL--;
	WR.BC--;
	valFlag(ctx, F_PV, WR.BC != 0);
	if(carry)
		setFlag(ctx, F_C);
	else
		resFlag(ctx, F_C);
	valFlag(ctx, F_5, value & (1 << 1));
	valFlag(ctx, F_3, value & (1 << 3));
}
...
static void CPI (Z80Context* ctx)
{
	...
	WR.BC--;
	valFlag(ctx, F_PV, WR.BC != 0);
	valFlag(ctx, F_C, carry);
	valFlag(ctx, F_5, value & (1 << 2));
	valFlag(ctx, F_3, value & (1 << 3));
}
...
static void IND (Z80Context* ctx)
{
	...
	WR.HL--;
	BR.B = doIncDec(ctx, BR.B, ID_DEC);
	valFlag(ctx, F_N, (val & 0x80) != 0);
	int flagval = val + ((BR.C - 1) & 0xff);
	valFlag(ctx, F_H, flagval > 0xff);
	valFlag(ctx, F_C, flagval > 0xff);
	valFlag(ctx, F_PV, parityBit[(flagval & 7) ^ BR.B]);
}
...
static void LD_A_I (Z80Context* ctx)
{
	...
	resFlag(ctx, F_H);
    resFlag(ctx, F_N);
	valFlag(ctx, F_PV, ctx->IFF2);
	valFlag(ctx, F_S, (BR.A & 0x80) != 0);
	valFlag(ctx, F_Z, (BR.A == 0));
...
static void LDD (Z80Context* ctx)
{
	...
	WR.HL--;
	WR.BC--;
	valFlag(ctx, F_5, ((BR.A + val) & 0x02) != 0);
	valFlag(ctx, F_3, ((BR.A + val) & F_3) != 0);
	resFlag(ctx, F_H);
    resFlag(ctx, F_N);
	valFlag(ctx, F_PV, WR.BC != 0);
}
...
static void OUTD (Z80Context* ctx)
{
	...
	valFlag(ctx, F_N, value & 0x80);
	valFlag(ctx, F_H, flag_value > 0xff);
	valFlag(ctx, F_C, flag_value > 0xff);
	valFlag(ctx, F_PV, parityBit[(flag_value & 7) ^ BR.B]);
	adjustFlags(ctx, BR.B);
}
